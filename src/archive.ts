...
function decodeBase64(value: Uint8Array): Uint8Array | undefined {
  ...
  // RFC 4648 defines the empty string as the base64 representation of an
  // empty byte sequence. Empty attachments are still files and must be kept.
  if (!clean) return new Uint8Array();
  if (clean.length % 4 !== 0 || !/^[A-Za-z0-9+/]*={0,2}$/.test(clean) || /=/.test(clean.slice(0, -2))) return undefined;
